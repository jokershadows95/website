...
****************************************************************************/
  ...
  $string = '[%time%] ID: %user% HOST: %host% IP: %remote_ip% ( %forwarded_for% ), - %url% -> Successed login'; // z konfiguracji odczytanie formatu zapisu logu
  ...
  $string = str_replace('%user%', (isset($_SESSION) ? $_SESSION['userinfo'] : 0), $string); // te linie możesz usunąć - obowiązuje jedynie w projekcie Coyote
